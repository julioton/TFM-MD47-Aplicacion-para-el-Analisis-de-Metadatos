...
def embed(imgFile, payload, password):
    ...
    print
    "[*] Input image size: %dx%d pixels." % (width, height)
    max_size = width * height * 3.0 / 8 / 1024  # max payload size
    print
    "[*] Usable payload size: %.2f KB." % (max_size)

    f = open(payload, "rb")
    data = f.read()
    f.close()
    print
    "[+] Payload size: %.3f KB " % (len(data) / 1024.0)
    ...
    print
    "[+] Encrypted payload size: %.3f KB " % (payload_size)
    ...
    print
    "[+] %s embedded successfully!" % payload
...
def extract(in_file, out_file, password):
    ...
    print
    "[+] Written extracted data to %s." % out_file
...
def usage(progName):
    ...
    "Usage:"
    print
    "  %s hide <img_file> <payload_file> <password>" % progName
    print
    "  %s extract <stego_file> <out_file> <password>" % progName
    print
    "  %s analyse <stego_file>" % progName
    sys.exit()
